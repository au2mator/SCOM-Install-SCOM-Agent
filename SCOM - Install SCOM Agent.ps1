...
# Code Template V 1.1
# URL: https://au2mator.com/install-scom-agent-scom-self-service-with-au2mator/
# Github: https://github.com/au2mator/SCOM-Install-SCOM-Agent
#################
...
[string]$LogPath = "C:\_SCOworkingDir\TFS\PS-Services\SCOM - Install SCOM Agent"
[string]$LogfileName = "Install SCOM Agent"
...
$SCOMManagementServer=Get-SCOMManagementServer -Name $OpsmgrServer

if ($c_AgentType -eq "Full")
{
    Write-au2matorLog -Type INFO -Text "Try to install Full Agent"

    Install-SCOMAgent -DNSHostName $c_Computer -PrimaryManagementServer $SCOMManagementServer -Confirm:$false
}
elseif ($c_AgentType -eq "Agentless")
{
    Write-au2matorLog -Type INFO -Text "Try to add agentless Agent"
    
    Add-SCOMAgentlessManagedComputer -DNSHostName $c_Computer -ManagedByManagementServer $SCOMManagementServer -Confirm:$false 
}


if ($ErrorCount -eq 0) {
    $au2matorReturn = "SCOM Agent deployed"
    $AdditionalHTML = "
        "
    $Status = "COMPLETED"
}
else {
    $au2matorReturn = "failed to deploy Agent, Error: $Error"
